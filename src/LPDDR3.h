...
public:
    ...
    LPDDR3(const std::string& org_str, const std::string& speed_str);
    ...
    /* Prerequisite */
    std::function<Command(DRAM<LPDDR3>*, Command cmd, int)> prereq[int(Level::MAX)][int(Command::MAX)];

    // SAUGATA: added std::function object container for row hit status
    /* Row hit */
    std::function<bool(DRAM<LPDDR3>*, Command cmd, int)> rowhit[int(Level::MAX)][int(Command::MAX)];
    std::function<bool(DRAM<LPDDR3>*, Command cmd, int)> rowopen[int(Level::MAX)][int(Command::MAX)];
    ...
    /* Lambda */
    std::function<void(DRAM<LPDDR3>*, int)> lambda[int(Level::MAX)][int(Command::MAX)];
